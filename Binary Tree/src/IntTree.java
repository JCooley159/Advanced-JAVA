...
public class IntTree {
    ...
    private String toString(IntTreeNode root, int sizeAboveLeft, int level, int size, int height, int width) {
        // System.out.println("toString(root, " + sizeAboveLeft + ", " + level + ", " +
        // size + ", " + height + ", " + width + ")");
        if (root == null) {
            return "";
        } else {
    ...
    private IntTreeNode fromString(StringBuilder s) {
        ...
        if (next.length() == 0 || next.equals("null")) {
            ...
            return new IntTreeNode(data, fromString(new StringBuilder(leftStr)),
                    fromString(new StringBuilder(rightStr)));
        }
    }

    // returns string representation of next complete node or data value from given
    // text
    ...
    // a traversal example to help you
    public int countEvenBranches() {
        return countEvenBranches(overallRoot);
    }

    protected int countEvenBranches(IntTreeNode root) {
        if (root == null) {
            return 0; // if DNE, then return 0 to count
        } else if (root.left == null && root.right == null) {
            return 0; // if no children, then return 0 to count
        } else // else, it has children and do this:
        {
            if (root.data % 2 == 0) // if the data is EVEN
            {
                // ADD 1 to count + count children
                return 1 + countEvenBranches(root.left) + countEvenBranches(root.right);
            } else // else the data is ODD
            {
                // ADD 0 to count + count children
                return 0 + countEvenBranches(root.left) + countEvenBranches(root.right);
            }
        }
    ...
    public int sumNodes(IntTreeNode node) {
        // add code here to sum the values of all the integers stored in the tree
        // you may add additional methods as needed

        if (node == null) {
            return 0; // if DNE, then return 0 to count
        } else // else, it exists and do this:
        {
            // ADD data to count + count children
    ...
    public int countLeftNodes(IntTreeNode node) {
        // add code here to count how many left children are in the tree. A left child
        // is
        // a node that appears as the root of the left-hand subtree of another node.
        // An empty tree has 0 left nodes. // you may use additional methods

        ///// I could initialize a variable here and have it return @ the bottom
        ///// of the method, but because I don't want to use excess memory for
        ///// each call of the method, I'm leaving the int addition for runtime
        ///// when the stack of countLeftNodes() calls resolves. ~J.Cooley

        if (node != null && node.left != null) // else, node exists, and if its L Child exists:
        {

            // recurse on L Child // recurse on R Child // ADD 1 to #ofLeftChildren
            return countLeftNodes(node.left) + countLeftNodes(node.right) + 1;
        } else if (node != null && node.right != null) // else, node exists, and if its R Child exists:
        {
            // I don't want to run into any cases where a node has a R Child but the child's
            // children don't get counted
            return countLeftNodes(node.left) + countLeftNodes(node.right) + 0;
        } else // else, the node is null/DNE, or doesn't have any children, so return 0
        {
            return 0; // if node or root DNE, then return 0 to count
        }
    }
    ...
    public static class IntTreeNode {
        ...
        public String toString() {
            if (this.cycle()) {
                ...
                // recursively adds data around itself
